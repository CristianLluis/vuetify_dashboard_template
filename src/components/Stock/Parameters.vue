  ...
  v-data-iterator(
    ...
    template(slot="item" slot-scope="{ item }")
      div(class="ma-2 elevation-1")
        div(class="pa-2 d-flex align-top")
          v-flex(
...
<script>
import { capitalize, camel } from "@/utils/helpers";
...
export default {
  ...
  computed: {
    computedItems() {
      return this.items.map(item => {
        const newItem =
          item !== Object(item) ? { name: item } : Object.assign({}, item);
  ...
  methods: {
    genDescription(name, item) {
      ...
      const specialLevelDesc = `${this.namespace}.${this.type}.${
        this.target
      }['${name}']`;
    ...
    genType(type) {
      ...
      return type
        .map(t => {
          let type = capitalize(t);
          return this.$t(`Generic.Types.${type}`);
        })
        .join(", ");
    },
    genProps(props) {
