<template>
  <v-img :gradient="gradient" :height="`${height}px`" :src="backgroundImg">
    <v-container fill-height pa-0>
      <v-layout align-center>
        <v-flex xs12>
          <v-card class="text-xs-center margin-auto" width="320">
            <v-card-text>
              <img src="/static/vuse-oneline-black.svg" width="275">
              <p class="an-16 text-xs-center regular-text">Powered By VueJS</p>
              <v-btn
                color="act"
                class="white--text btn--xlarge"
                @click.native.stop="dialog.signin=true"
              >Sign In</v-btn>
              <v-btn
                color="sidebar"
                class="white--text btn--xlarge"
                @click.native.stop="dialog.signup=true"
              >Sign Up</v-btn>
            </v-card-text>
          </v-card>
        </v-flex>
    ...
    <v-dialog v-model="dialog.signin" max-width="320" scrollable>
      <v-card>
        <v-card-title class="pos-relative">
          <v-btn absolute flat icon class="pos-tr-1px" @click="dialog.signin= !dialog.signin">
            <v-icon>close</v-icon>
          </v-btn>
        </v-card-title>
    ...
    <v-dialog v-model="dialog.forgotpassword" max-width="320" scrollable>
      <v-card>
        <v-card-text class="no-mrpd">
          <forgot-password
            @close="dialog.forgotpassword = false"
    ...
    <v-snackbar v-model="snackbar.visibility" absolute top right :color="snackbar.color">
