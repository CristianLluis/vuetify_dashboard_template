  ...
  examples: [
    {
      ...
      customFilter: {
        header: "自动补全的自定义筛选",
        desc:
          "`filter` 属性可以用来自定义逻辑过滤每个单项，在这个例子中，我们按名称过滤选项。"
      },
      scopedSlots: {
        header: "范围插槽",
        desc:
          "通过使用 scoped 插槽, 你可以自定义选项的视觉输出，在这个例子中，我们将个人资料图片添加到小纸片（chips）和列表项中。"
      },
      customTextAndValue: {
        header: "自定义选项的文本和值",
        desc:
          "你可以指定选项数组中的文本与值字段对应的特定属性，在默认情况下是 **text** 和 **value**。在这个例子中，我们也可以用 `return-object` 属性来返回被选中的对象。"
      },
      tags: {
        header: "标签",
        desc:
          "使用标签，你可以允许用户创建新值，这些值可能不在提供的选项中。请记住，标签只支持原始项的数组，并且不能和 `item-text`, `item-value` 这些属性一起使用。"
      },
      asynchronous: {
        header: "异步选项",
        desc:
          "有时候你需要根据搜索查询从外部获取数据。当你使用 `autocomplete` 属性时可以使用 `search-input` 属性加上 **.sync** 修饰符。 我们也可以使用新的 `cache-items` 属性。这将保持传给 `items` 属性一个所有选项中的唯一列表，并且在使用异步选项以及 **multiple** 属性时是必须的。"
      }
    }
  ],
  props: {
    ...
    disabled: "禁用输入",
    editable:
      ' 创建一个可编辑按钮 - <a href="https://material.io/guidelines/components/buttons.html#buttons-dropdown-buttons" target="_blank" rel="noopener">spec<a/>',
    filter: "用于过滤选项的函数",
    ...
    overflow:
      '创建一个溢出按钮 - <a href="https://material.io/guidelines/components/buttons.html#buttons-dropdown-buttons" target="_blank" rel="noopener">spec</a>',
    returnObject:
      "将选择器的行为更改为直接返回对象，而不是 item-value 指定的值",
    searchInput:
      "使用自动补全（autocomplete）属性，使用 .sync 修饰符从自动补全搜索框中捕获用户的输入",
    segmented:
      '创建一个分段按钮 - <a href="https://material.io/guidelines/components/buttons.html#buttons-dropdown-buttons" target="_blank" rel="noopener">spec</a>',
