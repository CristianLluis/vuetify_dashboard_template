  ...
  examples: [
    {
      ...
      absolute: {
        header: "绝对定位",
        desc:
          "菜单也可以使用`absolute`属性绝对定位在激活元素的顶部。尝试点击图片上的任何地方。",
        uninverted: true
      },
      absoluteWithoutActivator: {
        header: "没有激活时的绝对定位",
        desc:
          "菜单也可以通过`absolute`属性跟`position-x`和`position-y`属性一起使用。尝试右键点击图像上的仍和位置。",
        uninverted: true
      },
      hover: {
        header: "悬停",
      ...
      customTransition: {
        header: "自定义过渡",
        desc:
          "Vuetify带有3个标准过渡，**scale**、**slide-x**和**slide-y**。您也可以创建自己的并将其作为过渡参数传递。有关如何构建内置过渡的示例，请访问[此处](https://github.com/vuetifyjs/vuetify/blob/master/src/util/helpers.js#L13)。",
        uninverted: true
      },
      popover: {
        header: "弹出菜单",
