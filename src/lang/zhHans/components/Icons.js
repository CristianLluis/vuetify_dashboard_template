  ...
  examples: [
    {
      standard: {
        header: "标准",
        desc:
          "图标有两个主题（明亮和暗黑）和四个不同的大小（标准（standard）、中型（medium）、大型（large）和超大型（x-large））。"
      },
      fontAwesome: {
        header: "Font Awesome",
        desc:
          "[Font Awesome](http://fontawesome.io/icons/)也被支持，只需简单地使用`fa-`前缀的图标名称即可。请注意，您仍然需要在您的项目中包含Font Awesome图标。"
      },
      mdi: {
        header: "Material Design图标",
        desc:
          "[Material Design图标](https://materialdesignicons.com/)也被支持，您只需简单地使用`mdi-`前缀的图标名即可。请注意，您仍然需要在您的项目中包含MDI图标。"
      },
      color: {
        header: "色彩",
