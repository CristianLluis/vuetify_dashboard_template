export default {
  ...
  examples: [
    {
      standard: {
        header: "标准",
        desc:
          "标准的数据表格包含没有附加功能的数据。你可以通过 `hide-actions` 属性来控制是否需要显示分页信息，即 使用 `hide-action`将不分页，直接显示全部数据。"
      },
      noData: {
        header: "no-data 插槽",
      ...
      headers: {
        ...
        desc:
          "`items` 和 `headers` 可以接受 <kbd>td/th</kbd> 标签, 或者你想控制一整行，可以使用 <kbd>tr</kbd> 标签."
      },
      headerCell: {
      ...
      progress: {
        header: "progress 插槽",
        desc:
          "这里也有一个 `progress` 插槽让你可以自定义显示表格的加载状态，默认情况下这是一个`indeterminate` `v-progress-linear`（即不确定进度的线性进度条）。"
      },
      footer: {
        header: "footer 插槽",
        desc:
      ...
      expand: {
        header: "expand 插槽",
        desc:
          "`v-data-table` 组件还支持行展开，通过使用`expand` 插槽。 另外你可以使用 `expand` 属性来避免你点击另一行的时候已展开的行关闭。"
      },
      pageText: {
        header: "page-text 插槽",
  ...
  props: {
    "v-edit-dialog": {
      cancelText:
        "为取消按钮设置默认文本，当使用了 `large` 属性时（即有提交和取消按钮的对话框）。",
