  ...
  examples: [
    {
      ...
      validation: {
        header: "验证",
        desc:
          "Vuetify包含简单的验证通过使用 `rules` 属性，这个属性接受一个回调函数组，在验证规则时，当前的 v-model 值将被传递给回调函数，这个回调函数必须返回 `ture` 或 `String` 或者错误信息。"
      },
      ...
      requiredFields: {
        header: "必填字段",
        desc:
          "在这个例子中两个文本输入框是必填的，我们利用 **required** 属性为必填项设置星号并验证这些字段。"
      },
      hint: {
        header: "提示文本",
        desc:
      ...
      prefixesAndSuffixes: {
        header: "前缀和后缀",
        desc:
          " **prefix** 和 **suffix** 属性允许你在文本字段旁添加一段不可修改的文本。"
      },
      customValidation: {
        header: "自定义验证",
        desc:
          '虽然内置的 `v-form` 组件以及第三方插件比如 <a href="https://github.com/monterail/vuelidate" target="_blank" rel="noopener">vuelidate</a> or <a href="https://github.com/logaretm/vee-validate" target="_blank" rel="noopener">vee-validation</a> 可以帮助你简化验证过程，但你仍可以简单的自行控制它。'
      },
      textarea: {
        header: "多行文本框",
        desc: "多行文本框有一个可选样式。",
