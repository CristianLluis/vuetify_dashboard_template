export default {
  ...
  examples: [
    {
      avatarTwoLines: {
        header: "头像和两行内容的组合",
        desc:
          "列表可以包含一个items数组。如果给定一个数组，列表组件将依据给出的内容找到相关的类，你也可以在items数组定义列表组标题（headers）或分隔线（divider）。"
      },
      ...
      avatarThreeLines: {
        header: "头像和三行的组合",
        desc:
          '对于3行的列表，列表项副标题内容占两行的时候如果超出范围则以省略号形式截断文字，如果你需要超过3行，建议使用 <router-link to="/components/cards">card</router-link>组件。'
      },
  ...
  props: {
    avatar:
      "用来设置单行列表项的最小高度（即使用该属性后将有一个头像大小的最小高度）",
    dense: "减小列表块的最大宽度",
    ...
    twoLine: "增加列表块的高度到2行",
    inactive:
      "如果设置了该属性，即使有to/href或者@click，列表块也不会响应链接。"
  }
};
