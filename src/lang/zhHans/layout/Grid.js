export default {
  ...
  examples: [
    {
      grid: {
        header: "用例",
        desc:
          "`v-container`可以用于中心聚焦的页面，或者用`fluid`来扩展它的全部宽度。`v-layout`用于分隔各部分，并包含`v-flex`。您的布局结构如下，**v-container** &raquo; **v-layout** &raquo; **v-flex**。栅格链的每个部分都是一个弹性框元素，最后，`v-flex`会自动设置它的子元素获得<kbd>flex: 1 1 auto</kbd>。",
        uninverted: true
      },
      offset: {
        header: "偏移",
      ...
      directionAndAlign: {
        header: "方向和对齐",
        desc:
          "以各种方式指定方向和对齐方式。所有可用的<kbd>flex-box</kbd> api都可以通过直观的辅助器属性获得。",
        uninverted: true
      },
      rowColumnBreakpoint: {
        header: "行和列断点",
        desc:
          "更具分辨率动态更改布局。**（调整您的屏幕，并观看布局更改为一个`行`的小断点）**",
        uninverted: true
      },
      nestedGrid: {
        header: "嵌套栅格",
      ...
      tags: {
        header: "Html标签",
        desc:
          "有的时候您会想要将一个布局项指定为特定的标签，比如一个`section`或者`li`元素。"
      }
    }
  ],
  props: {
  ...
  breakpointText2:
    "该对象包含与您已经习惯使用的栅格系统相同的语义属性。让我们尝试一个真实世界的例子。您有一个`v-dialog`组件，您想要在移动设备上转换成一个**全屏**对话框。通常情况下，您需要将视野大小绑定在观察者上，和/或在页面加载时进行检查。",
  breakpointText3:
    "这是很多的样板文字。即使您选择使用内置的[v-resize](/directives/resizing)指令，仍然必须定义调整大小的方法。使用**断点**对象，您可以完全跳过这个逻辑并重新构建您的应用程序。"
};
