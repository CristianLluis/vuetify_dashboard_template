export default {
  ...
  howText:
    "将**margin**或者**padding**应用于一个元素，范围在 _0到5_ 之间。每个尺寸增量都设计成与常用Material Design间距对齐。这些类可以使用`{property}{direction}-{size}这个格式来应用。`.",
  propertyText: "占位符 _property_ 用来设置间距的类型：",
  properties: ["`m` - 对应 `margin`", "`p` - 对应 `padding`"],
  ...
  horizontalText:
    "对于具有指定宽度的块元素，可以使用`.mx-auto`来使它的内容水平居中"
};
