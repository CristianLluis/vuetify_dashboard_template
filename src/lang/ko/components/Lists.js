export default {
  ...
  examples: [
    {
      ...
      avatarThreeLines: {
        ...
        desc:
          "세 줄 단위 리스트에서, 서브타이틀은 세로로 두번째 줄에 고정(clamp)되고 나머지는 생략표시(ellipsis) 됩니다. 만약 3줄 이상이 필요하다면 [카드(card)](/ko/components/cards)를 쓰는 것을 권장합니다."
      },
  ...
  props: {
    ...
    group:
      "라우트 네임스페이스를 지정. 활성화 상태를 결정하기 위한 문자열이나 정규표현식.",
    ...
    inactive:
      "리스트 타일이 to/herf prop을 가지고 있거나 @click 핸들러를 가지고 있어도 링크로 렌더링 되지 않음",
