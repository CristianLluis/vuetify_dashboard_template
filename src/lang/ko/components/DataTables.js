export default {
  ...
  examples: [
    {
      ...
      headers: {
        ...
        desc:
          "`items` 과 `headers` 슬롯엔 <kbd>td/th</kbd> 태그 모음이나 전체 row 를 제어하고 싶을 경우 <kbd>tr</kbd> 태그를 넣습니다."
      },
      headerCell: {
      ...
      progress: {
        header: "슬롯: progress",
        desc:
          "`progress` 슬롯은 데이터 테이블의 `로딩` 상태 표시를 커스터마이즈 할 수 있습니다.\n기본 값은 `indeterminate` `v-progress-linear` 입니다."
      },
      footer: {
        header: "슬롯: footer",
        desc:
          "`footer` 슬롯으로 테이블에 별도의 기능을 추가할 수 있습니다. 예) 컬럼 필터링이나 검색"
      },
      expand: {
        header: "슬롯: expand",
        desc:
      ...
      paginate: {
        ...
        desc:
          "페이지네이션(Pagination)은 `pagination` prop을 통해 외부에서 제어될 수 있습니다.\n `.sync` 수식어(modifier)를 반드시 사용해야 합니다."
      },
      sort: {
        header: "외부 정렬 (External sorting)",
        desc:
          "정렬 또한 `pagenation` prop을 통해 외부에서 제어될 수 있습니다. `.sync` 수식어(modifier)를 반드시 사용해야 한다는 것을 기억하세요. \n `pagenation` prop으로 또한 기본 정렬 열(default sorted column) 을 정할 수도 있습니다."
      },
      server: {
        header:
  ...
  props: {
    "v-edit-dialog": {
      cancelText:
        "**large** prop을 사용할때 나오는 취소(cancel) 버튼의 기본 텍스트를 설정",
      lazy: "Mixins.Bootable.props.lazy",
      large:
        "대화찰(dialog)에 서브밋(submit)과 취소(cancel) 버튼을 추가합니다.",
      saveText:
        "**large** prop을 사용할때 나오는 저장(save) 버튼의 기본 텍스트를 설정",
