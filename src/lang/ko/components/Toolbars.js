  ...
  examples: [
    {
      usage: {
        header: "사용법 (Usage)",
        desc:
          "툴바는 여러 방법으로 쓰일 수 있는 유연한 컨테이너 입니다. 기본적으로 툴바는 데스크탑에서 64px, 세로 모바일에서 56px, 가로 모바일에서 48px 높이를 가집니다. 툴바와 함께 쓰일 수 있는 `v-toolbar-side-icon`, `v-toolbar-title`, `v-toolbar-items` 같은 여러 헬퍼 컴포넌트가 있습니다."
      },
      appBar: {
        header: "앱바 (App bar)",
        desc:
          "앱바는 어플리케이션의 주된 툴바입니다. 앱 바는 아이콘, 메뉴 등을 포함할 수 있습니다."
      },
      appBarItems: {
        header: "앱바와 아이템 (App bar with items)",
        desc:
          "`v-toolbar-items` 컴포넌트를 사용해서 일반적인 `v-btn` 컴포넌틀 앱바 아이템으로 사용할 수 있습니다. 각 버튼에 `flat` prop 을 적용하는 것을 잊지 마세요."
      },
      appBarExtension: {
        header: "앱바와 확장 (App bar with extension)",
        desc:
          "더 두드러지는 바를 만들기 위해 확장을 사용할 수 있습니다. An extension can be used that creates a much more prominent bar. If a title is placed within the extension, it will automatically be shifted to the right to match the normal location it would be at in the main bar."
      },
      ...
      prominent: {
        ...
        desc:
          "두드러지는 툴바는 확정된 공간에 요소를 추가할 수 있는 슬롯을 추가합니다. 컨텐츠와 확장 공간은 64px 높이로 고정됩니다."
      },
      dense: {
      ...
      scrolling: {
        ...
        desc:
          "툴바는 스크롤시에 트렌지션되어 사라질 수(transition off) 될 수 있습니다. 이 예제는 어떻게 스크롤 타겟을 정의할 수 있는지(기본값은 window) 보여줍니다. 이 예제에서 일반적인 어플리케이션에 아마도 필요없을 특별한 마크업이 사용되었다는 것을 기억하세요"
      },
      appBarMenu: {
      ...
      systemBarsStatusWindow: {
        ...
        desc:
          "시스템바는 모바일과 데스크탑에서 알람 아이콘을 보여주는데 유용합니다. For live examples of, head to the pre-defined layouts."
      }
    }
  ],
  props: {
    ...
    extended:
      "툴바가 슨롯을 사용하지 않고 확장을 생성하도록 강제 (Force the toolbar to generate the extension without using the slot)",
    flat: "툴바의 박스 그림자를 제거",
