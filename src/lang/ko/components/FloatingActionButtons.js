  ...
  examples: [
    {
      promotedAction: {
        ...
        desc:
          "플로팅 액션 버튼을 구성요소에 결합하면 어플리케이션에서 장려되는 액션을 강조할 수 있습니다. 대부분의 경우 기본 크기가 사용되지만, 다른 요소와의 조화를 위해 `small` 변형이 있습니다."
      },
      small: {
      ...
      displayAnimation: {
        ...
        desc:
          "처음 보여질때, 플로팅 액션 버튼은 스크린 위로 애니매이션 효과와 함께 나타납니다. 여기서 v-show 와 함께 `v-fab-transition`이 사용되었습니다. 또한 Vuetify 에서 재공하는 또는 직접 만든 커스텀 트랜지션을 사용할 수도 있습니다."
      },
      lateralScreens: {
        header: "가로 화면 (Lateral screens)",
        desc:
          "버튼의 기본 액션을 바꿀땐, 바뀌는 것을 강조하기 위한 트랜지션을 사용하는 것이 권장됩니다. 이를 위해 `key` prop에 Vue 트랜지션 시스템을 전환을 지시하기 위한 일련의 데이터를 연결합니다. 커스텀 트랜지션을 사용할 수 있지만 이때 `mode` prop 을 **out-in**으로 설정해야 합니다."
      },
      speedDial: {
