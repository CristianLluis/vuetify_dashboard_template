export default {
  ...
  examples: [
    {
      ...
      validation: {
        ...
        desc:
          "Vuetify는 `rules` prop 을 통해 간단한 검증(validation)을 제공합니다. `rules` prop은 콜백 배열을 받습니다. 규칙들(rules)을 검증할 때, 현재 v-model 값이 콜백으로 전달됩니다. 이 콜백은 `true` 나 에러 메시지로 `String`을 반환해야 합니다."
      },
      ...
      requiredFields: {
        ...
        desc:
          "이 예제에서는 두 개의 필수 텍스트 필드가 사용되었습니다. 레이블에 애스터리스크를 적용하고 필드 검증을 하기 위해 **required** prop을 활용하였습니다."
      },
      hint: {
      ...
      prefixesAndSuffixes: {
        ...
        desc:
          "**prefix**와 **suffix** 프로퍼티는 텍스트 필드에 편집 불가능한 인라인 텍스트를 앞이나 뒤에 붙일 수 있습니다."
      },
      customValidation: {
        header: "사용자 검증 (Custom validation)",
        desc:
          "내장된 `v-form`이나 [vuelidate](https://github.com/monterail/vuelidate),[vee-validation](https://github.com/logaretm/vee-validate)같은 서드파티 플러그인은 검증 프로세스를 구성하는 것을 도와주지만 동시에 간단한 사용자 검증을 선택 할 수도 있습니다."
      },
      textarea: {
        header: "Textarea",
  ...
  props: {
    autoGrow:
      "인풋의 자동 확장(Auto-grows). 이 옵션은 **v-model**을 필요로 합니다.",
    ...
    noResize:
      "리사이즈 핸들을 제거. **multi-line**, **textarea** props 이 지정된 경우에만 적용",
    prefix: "접두사(prefix) 표시",
    rows: "textarea 의 열(row) 수",
    rowHeight:
      "사용자 _row-height_ 를 지정. **multi-line**, **textarea** 을 사용할때 인풋의 높이를 결정하기 위해 사용됨",
