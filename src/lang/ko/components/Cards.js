export default {
  ...
  components: ["v-card", "v-card-media", "v-card-title", "v-card-actions"],
  examples: [
    {
      components: {
      ...
      horizontal: {
        ...
        desc:
          "`v-flex`를 이용해 커스텀 수평 카드를 만들 수 있습니다. `v-card-media`가 컨테이너를 덮지 않고 딱 맞게 들어갈 수 있도록 `contain` 속성(property)를 사용해서 크기를 줄이세요(shrink)."
      },
      grids: {
        header: "그리드 (Grids)",
        desc:
  ...
  props: {
    contain:
      "백그라운드 사이즈 (background-size) 를 컨테이너에 맞게 조정합니다.",
    flat: "카드 박스의 그림자를 제거",
    height: "카드의 높이를 설정",
