export default {
  ...
  headerText:
    "`v-date-picker`는 여러 다른 Vuetify 컴포넌트에서 쓰일 수 있는 독립적 컴포넌트입니다.\n 이 컴포넌트는 날짜/월을 선택할 수 있는 시각적 표현을 제공합니다.",
  ...
  examples: [
    {
      ...
      dateColorable: {
        ...
        desc:
          "날짜 선택기의 색은 `color` 와 `head-color` prop으로 설정할 수 있습니다. `if-header-color` prop이 없으면 `color` prop의 값이 사용됩니다."
      },
      dateDialogAndMenu: {
        header:
      ...
      monthColorable: {
        ...
        desc:
          "Month picker의 색은 `color` 와 `head-color` prop으로 설정할 수 있습니다. `if-header-color` prop이 없으면 `color` prop의 값이 사용됩니다."
      },
      monthDialogAndMenu: {
        header:
