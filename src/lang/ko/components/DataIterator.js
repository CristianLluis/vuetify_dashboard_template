export default {
  ...
  headerText:
    "`v-data-iterator` 컴포넌트는 데이터를 보여주는데 사용합니다. 주요 기능은 `v-data-table`과 공유합니다.\n정렬(sorting), 검색(searching), 페이지네이션(pagination), 선택(selection) 등이 가능합니다.",
  ...
  examples: [
    {
      simple: {
        ...
        desc:
          "`v-data-iterator`로 데이터가 어떻게 보일지 정확하게 커스터마이즈 할 수 있습니다. 이 예제에서는 카드(Cards)와 그리드 리스트(Grid lists)를 사용합니다.\n`content-tag` prop을 (`content-class`, `content-props`와 함께) 사용하여 아이템들을 둘러싼 요소가 어떻게 보여야할지 지정할 수 있습니다."
      }
    }
  ],
  props: {
