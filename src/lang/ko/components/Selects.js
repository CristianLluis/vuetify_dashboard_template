  ...
  examples: [
    {
      ...
      multiple: {
        ...
        desc:
          "multi-select 에서 선택된 항목들을 보여줄때 `v-chip`을 사용할 수 있습니다."
      },
      autocomplete: {
      ...
      customFilter: {
        ...
        desc:
          "`filter` prop 을 사용하면 각각의 항목을 커스텀 로직으로 필터링할 수 있습니다. 예를 들어 항목을 이름으로 필터링 합니다."
      },
      scopedSlots: {
        header: "스코프드 슬롯 (Scoped slots)",
        desc:
          "스코프드 슬롯의 강력함으로, 셀렉트의 아웃풋을 다양하게 표시할 수 있습니다. 예로 칩(chip)과 리스트 항목에 프로필 사진을 추가합니다."
      },
      customTextAndValue: {
        header:
          "항목 텍스트와 값의 커스터마이즈 (Customized item text and value)",
        desc:
          "텍스트와 값 필드에 해당하는 항목 배열 내의 특정 속성을 지정할 수 있습니다. 기본값은 **text** 와 **value** 입니다. 이 예제에서는 선택된 아이템의 전체 오브젝트를 반환하는 `return-opject` prop을 사용합니다.."
      },
      tags: {
        header: "태그 (Tags)",
        desc:
          "태그를 사용하여 사용자가 아이템리스트에 아직 없는 새로운 값을 만들 수 있게 합니다. 태그는 **primitive** 항목들의 배열만 지원하고 `item-text` 나 `item-value` 와 같은 prop들과는 사용할 수 없다는 점을 명심하세요."
      },
      asynchronous: {
        header: "비동기 항목 (Asynchronous items)",
        desc:
          "때로, 검색 쿼리에 따라 외부에서 데이터를 가져올 필요가 있습니다. `autocomplete` prop을 사용할때  `search-input` prop을 **.sync** 수식어와 사용하세요. 또한 새로운 `cache-item` prop을 사용하면 비동기 아이템과 **복수의** prop을 사용할때 `items` prop으로 넘겨지고 **REQUIRED** 인 모든 항목의 유니크 리스트를 보관합니다."
      }
    }
  ],
  props: {
    ...
    cacheItems:
      "**items** prop으로 넘겨진 모든 항목의 _유니크한_ 로컬 사본을 보관합니다.",
    ...
    editable:
      " 편집 버튼을 생성 - [스펙](https://material.io/guidelines/components/buttons.html#buttons-dropdown-buttons)",
    ...
    multiLine:
      "셀렉트 컴포넌트가 포커스 되거나 dirty(?) 할때 레이블이 떠다니도록(float) 만듬.",
    ...
    overflow:
      "오버플로우 버튼을 생성 - [스펙](https://material.io/guidelines/components/buttons.html#buttons-dropdown-buttons)",
    returnObject:
    ...
    segmented:
      "Creates a segmented button - [spec](https://material.io/guidelines/components/buttons.html#buttons-dropdown-buttons)",
    tags:
