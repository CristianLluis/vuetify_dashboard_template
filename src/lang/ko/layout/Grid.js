export default {
  ...
  headerText:
    "Vuetify 12 포인트 그리드 시스템을 사용합니다. <kbd>flex-box</kbd>를 사용하는 이 그리드는 어플리케이션 컨텐츠의 레이아웃에 사용됩니다. 여기엔 여러 화면 크기와 방향에 대응하는 5가지 타입의 미디어 브레이크포인트가 있습니다. 그리드 컴포넌트의 prop들은 사실 정의된 속성에 따라 파생되는 (css) 클래스들입니다. 이를 이용해 이들 (css) 헬퍼 클래스들을 prop 형태로 쉽게 사용할 수 있습니다. 물론 이 클래스들은 여전히 어디든지 사용할 수 있습니다.",
  ...
  examples: [
    {
      ...
      directionAndAlign: {
        ...
        desc:
          "방향과 정렬을 다양한 방법으로 지정합니다. 가능한 모든 <kbd>flex-box</kbd> api 는 직관적인 헬퍼 prop을 통해 가능합니다.",
        uninverted: true
      },
      rowColumnBreakpoint: {
  ...
  props: {
    ...
    "v-flex": {
      ...
      "order-(size)(1-12)":
        "order-xs: extra small, order-sm: small, order-md: medium, order-lg: large, order-xl: extra large. Example: order-xs3",
      "(size)(1-12)":
  ...
  breakpointText3:
    'That\'s a lot of boilerplate to write. Even if you opt to use the built in <a href="/directives/resizing">v-resize</a> directive, you are still going to have to define a resize method. With the **breakpoint** object you can completely skip this logic and get back to building your application.'
};
