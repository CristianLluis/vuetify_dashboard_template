export default {
  header: "Bottom sheet",
  headerText:
    "Bottom sheet はスクリーンの下部からスライドするように修飾された `v-dialog` で `v-bottom-nav` と似ています。 ボトムナビゲーションコンポーネントは、ボタンと特定のアプリケーションレベルのアクション用ですが、 Bottom sheet にはなんでも含めることができます。",
  ...
  examples: [
    {
      standard: {
        header: "基本的な表示",
        desc:
          "ここではアプリケーションに存在する可能性のあるアクションの一覧の例を示します。"
      },
      inset: {
