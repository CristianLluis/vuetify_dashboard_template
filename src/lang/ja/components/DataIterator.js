  ...
  examples: [
    {
      simple: {
        header: "シンプルな用法",
        desc:
          "`v-data-iterator` コンポーネントはデータをどのように表示するか細かくカスタマイズすることができます。この例では、カードとグリッドリストを使用しています。項目のラッパー要素がどのような見た目にするかを決定するため、`content-tag` prop (加えて `content-class` と `content-props`) を使用できます。"
      }
    }
  ],
  props: {
