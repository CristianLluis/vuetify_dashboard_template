export default {
  ...
  examples: [
    {
      standard: {
        header: "基本的な使い方",
        desc:
          "基本的な Data table は付加的な機能を含まないデータを格納します。 `hide-actions` prop を使用することで、ページネーションを制御するテーブルアクションの表示を外すことができます。"
      },
      noData: {
        header: "スロット: no-data",
        desc:
      ...
      headers: {
        ...
        desc:
          "`items` と `headers` スロットは <kbd>td/th</kbd> タグを受け付けることができます。行の全体を制御したい場合は、 <kbd>tr</kbd> を受け付けることができます。"
      },
      headerCell: {
  ...
  props: {
    "v-edit-dialog": {
      cancelText:
        "**large** prop を使用した際に表示されるキャンセルボタンのデフォルトテキストを設定します。",
      lazy: "Mixins.Bootable.props.lazy",
      large: "ダイアログの送信ボタンとキャンセルボタンに適用されます。",
      saveText:
        "**large** prop を使用した際に表示される保存ボタンのデフォルトテキストを設定します。",
