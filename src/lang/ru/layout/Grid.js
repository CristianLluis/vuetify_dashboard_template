export default {
  ...
  examples: [
    {
      ...
      order: {
        header: "Порядок",
        desc:
          "Вы можете управлять порядком элементов сетки. Как и в случае смещений, вы можете устанавливать разный поряд для разных размеров. Дизайн специализированных макетов экрана, которые подходят для любого приложения.",
        uninverted: true
      },
      directionAndAlign: {
        header: "Направление и выравнивание",
        desc:
          "Назначьте направление и выравнивание различными способами. Все доступные <kbd> flex-box </kbd> api доступны через интуитивные вспомогательные props.",
        uninverted: true
      },
      rowColumnBreakpoint: {
      ...
      tags: {
        header: "Html tags",
        desc:
          "Иногда вам нужно указать элемент макета в качестве определенного тега, например, элемент `section` или `li`."
      }
    }
  ],
  props: {
    ...
    wrap:
      "Позволяет детям обертываться внутри контейнера, если элементы используют более 100%.",
    "v-container": {
  ...
  breakpointText1:
    "Vuetify преобразует доступные точки останова в доступный объект из вашего приложения. Это позволит вам назначать / применять определенные свойства и атрибуты на основе размера viewport. Доступ к объекту можно получить из:",
  breakpointText2:
  ...
  breakpointText3:
    'Это много шаблонов для написания. Даже если вы решите использовать встроенную директиву <a href="/directives/resizing"> v-resize </a>, вам все равно придется определять метод изменения размера. С объектом **breakpoint** вы можете полностью пропустить эту логику и вернуться к созданию своего приложения.'
};
