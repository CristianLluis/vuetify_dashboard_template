export default {
  header: "Расстояние",
  headerText:
    "Обновите макет, не создавая новые классы. Помощники интервалов полезны для изменения **padding** и **margin** элемента.",
  toc: [
    {
      text: "Введение",
      href: "introduction"
  ...
  howText:
    "Примените **margin** или **padding** к элементу в диапазоне от _0 до 5_. Инкремент каждого размера был разработан для согласования с общим интервалом Material Design. Эти классы могут применяться с использованием следующего формата `{property} {direction} - {size}`.",
  propertyText: "_property_ применяет тип интервала:",
  properties: ["`m` - применяет `margin`", "`p` - применяет `padding`"],
  directionText:
    "Параметр _direction_ обозначает сторону, к которой относится свойство:",
  directions: [
  ...
  horizontalText:
    "Для блоков с заданной шириной вы можете применить `.mx-auto` для горизонтального центрирования содержимого."
};
