  ...
  examples: [
    {
      activator: {
        header: "Activator",
        desc:
          "Не забудьте поставить элемент, который активирует меню в `activator` slot.",
        uninverted: true
      },
      absolute: {
        header: "Абсолютное положение",
        desc:
          "Меню также могут быть помещены абсолютно поверх элемента активатора с помощью `absolute` prop. Попробуйте щелкнуть в любом месте изображения.",
        uninverted: true
      },
      absoluteWithoutActivator: {
        header: "Абсолютное положение без активатора",
        desc:
          "Меню также можно использовать без активатора, используя `absolute` вместе с реквизитами `position-x` и `position-y`. Попробуйте щелкнуть правой кнопкой мыши в любом месте изображения.",
        uninverted: true
      },
      hover: {
        header: "Hover",
      ...
      customTransition: {
        ...
        desc:
          'Vuetify поставляется с 3 стандартными переходами, **scale**, **slide-x** и **slide-y**. Вы также можете создать свой собственный и передать его в качестве аргумента перехода. Для примера того, как построены стоковые переходы,<a href="https://github.com/vuetifyjs/vuetify/blob/master/src/util/helpers.js#L13" target="_blank" rel="noopener"> посетите это</a>.',
        uninverted: true
      },
      popover: {
        header: "Popover menu",
