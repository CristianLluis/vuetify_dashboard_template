...
import componentsJson from "@/data/json/VuseComponents";
  ...
  computed: {
    ...
    computedComponents() {
      ...
      if (component) {
        ...
        component = `${this.computedSection}.${
    ...
    examples() {
      ...
      return component.examples.map(example => {
        ...
        const namespace = `${this.computedSection}.${
          this.computedComponent
        }.examples.${file}`;

        return {
          file,
          desc: has(this.localeMessages[this.locale], `${namespace}.desc`)
            ? this.$t(`${namespace}.desc`)
            : null,
          header: has(this.localeMessages[this.locale], `${namespace}.header`)
            ? this.$t(`${namespace}.header`)
            : null,
          newIn,
          git: this.$t(`${namespace}.git`) === "true",
          gitalt: this.$t(`${namespace}.gitalt`),
          codepan: this.$t(`${namespace}.codepan`) === "true"
        };
      });
    },
    exists() {
      return (
        (this.computedComponents || []).length > 0 ||
